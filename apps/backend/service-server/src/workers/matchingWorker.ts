...
export const matchingWorker = new Worker<MatchingJobData>(
  ...
  async (job: Job<MatchingJobData>) => {
    ...
    const threshold = parseFloat(env.MATCHING_SCORE_THRESHOLD || '0.2');

    try {
      switch (type) {
        case 'full':
          // Process all matches
          const notifications = await processAllMatches({ batchSize: 20, enableLogging: true });
        ...
        case 'proposal':
          ...
          for (const notification of proposalNotifications) {
            try {
              await sendMatchNotification(notification);
            } catch (error) {
              console.error(`[Matching Worker] Failed to send notification:`, error);
        ...
        case 'team':
          ...
          for (const notification of teamNotifications) {
            try {
              await sendMatchNotification(notification);
            } catch (error) {
              console.error(`[Matching Worker] Failed to send notification:`, error);
