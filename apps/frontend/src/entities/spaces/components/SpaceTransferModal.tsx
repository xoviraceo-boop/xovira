...
export function SpaceTransferModal({ spaceId, spaceName, open, onOpenChange, onSuccess }: SpaceTransferModalProps) {
    ...
    // Fetch space data to get current owner and validate permissions
    const { data: space, isLoading: isLoadingSpace } = trpc.space.get.useQuery(
        { id: spaceId || "" },
        { enabled: !!spaceId && open }
    );

    // Server-side search for members (only when search query exists)
    // TODO: Replace with dedicated searchMembers endpoint
    const { data: searchResults, isLoading: isSearching } = trpc.space.get.useQuery(
        { id: spaceId || "" },
        {
            enabled: !!spaceId && open && debouncedSearch.length > 0,
            staleTime: 30000 // Cache for 30 seconds
        }
    );

    // Get members from space data
    const allMembers = (space as any)?.members?.map((member: any) => {
        const u = member.user;
        const name = u?.name ||
            u?.username ||
    ...
    // Filter members client-side (temporary until server-side search is implemented)
    const currentOwnerId = (space as any)?.createdBy;
    ...
    // Transfer ownership mutation with proper error handling
    const transferOwnership = trpc.space.update.useMutation({
        onSuccess: () => {
            toast({
                title: "Ownership transferred successfully",
                description: `${selectedMember?.name} is now the owner of ${spaceName}`
            });
            ...
            <Dialog open={open && !showConfirmation} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Transfer ownership of "{spaceName}" to another member
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-amber-800 mt-1">
                                        This action will transfer full control of this space. You will be downgraded to Admin role.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="space-y-3 pt-2">
                            <p>
                                You are about to transfer ownership of <strong>"{spaceName}"</strong> to:
                            </p>
