...
import { Tool, ToolCall } from '../types/types';
...
export async function executeTool(
  ...
  try {
    // Get tool definition
    const tool = await getToolById(toolCall.toolName);
    if (!tool) {
    ...
    switch (toolCall.toolName) {
      case 'createTask':
        result = await executeCreateTask(toolCall.parameters, userId, workspaceId);
        break;
      case 'updateTask':
      ...
      case 'createDocumentation':
        result = await executeContentCreationTool(toolCall.toolName, toolCall.parameters, userId, workspaceId);
        break;
      ...
      case 'refactorCode':
      case 'debugCode':
        result = await executeCodeOperationTool(toolCall.toolName, toolCall.parameters, userId, workspaceId);
        break;
      ...
      case 'clickElement':
      case 'scrapeData':
        result = await executeBrowserAutomationTool(toolCall.toolName, toolCall.parameters, userId, workspaceId);
        break;

      // Media Generation
      case 'generateImage':
      case 'generateVideo':
        result = await executeMediaGenerationTool(toolCall.toolName, toolCall.parameters, userId, workspaceId);
        break;
      ...
      case 'writeFile':
      case 'listFiles':
        result = await executeFileOperationTool(toolCall.toolName, toolCall.parameters, userId, workspaceId);
        break;
