...
export default function ProjectCard({ item, onOpen }: { item: any; onOpen?: (id: string) => void; }) {
  return (
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          {/* Add min-w-0 here to allow the text to shrink and wrap */}
      ...
      <CardContent>
        <div className="mb-3 flex flex-wrap gap-2 text-xs">
          {(item.tags || item.keywords || []).slice(0, 4).map((t: string) => (
