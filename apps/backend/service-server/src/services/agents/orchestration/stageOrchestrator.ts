...
import { ConversationStage, AgentDraft } from '../agentBuilderStateService';
...
import { StageReadinessSchema } from '../schemas';
...
Should we stay in "${currentStage}" or progress?`,
    ...
    const estimatedTokens = this.tokenBudgetManager.estimateTokens(JSON.stringify(progressionMessages)) + 300;
    const budgetCheck = await this.tokenBudgetManager.checkBudget('stageProgression', estimatedTokens);
