...
export function TeamTransferModal({ teamId, teamName, open, onOpenChange, onSuccess }: TeamTransferModalProps) {
    ...
    // Fetch team data to get current members and validate permissions
    const { data: team, isLoading: isLoadingTeam } = trpc.team.get.useQuery(
        { id: teamId || "" },
        { enabled: !!teamId && open }
    );

    // Get members from team data
    const allMembers = (team as any)?.members?.map((member: any) => {
        const u = member.user;
        const name = u?.name ||
            u?.username ||
    ...
    const currentOwnerId = (team as any)?.createdBy;
    ...
    const handleConfirmTransfer = async () => {
        ...
        try {
            const res = await fetch(`/api/permissions/team/${teamId}/transfer`, {
                method: 'POST',
            ...
            toast({
                ...
                description: `${selectedMember?.name} is now the owner of ${teamName}`
            });
            ...
            <Dialog open={open && !showConfirmation} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Transfer ownership of "{teamName}" to another member
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-amber-800 mt-1">
                                        This action will transfer full control of this team. You will be downgraded to Admin role.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="space-y-3 pt-2">
                            <p>
                                You are about to transfer ownership of <strong>"{teamName}"</strong> to:
                            </p>
