...
export function TeamSettingsSidebar({ teamId, open, onClose, inline }: Props) {
    ...
    const handleSave = async () => {
        try {
            await updateMutation.mutateAsync({ id: teamId, name: name.trim() || undefined, description: description.trim() || undefined });
            await utils.team.get.invalidate({ id: teamId });
        ...
        <div className={`${inline ? 'absolute inset-y-0 right-0' : 'fixed inset-y-0 right-0'} z-[60] w-auto min-w-[20rem] max-w-sm bg-background shadow-xl transition-transform duration-300 ${open ? '-translate-x-14' : 'translate-x-full'}`}>
            ...
            <div className="p-4 space-y-4">
                <div className="space-y-2">
                    <label htmlFor="team-name" className="text-sm font-medium">Title</label>
                    <Input id="team-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Team title" />
                </div>
                <div className="space-y-2">
                    <label htmlFor="team-description" className="text-sm font-medium">Description</label>
                    <Textarea id="team-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" />
                </div>
