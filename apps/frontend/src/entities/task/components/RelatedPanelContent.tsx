...
import { ArrowLeftRight, Search, ChevronDown, ChevronRight, Plus, ExternalLink, X, Maximize2, Flag } from 'lucide-react';
...
interface RelatedPanelContentProps {
    taskId: string;
    workspaceId: string;
    /** All dependencies (for custom relationship sections - we use FINISH_TO_FINISH as "related" tasks) */
    items: Array<{
        id: string;
        taskId: string;
        dependsOnId: string;
...
export function RelatedPanelContent({ taskId, workspaceId, items, customTypes }: RelatedPanelContentProps) {
    const [pickerOpen, setPickerOpen] = React.useState(false);
    const [collapsedIds, setCollapsedIds] = React.useState<Record<string, boolean>>({});

    const linkedItems = React.useMemo(() => items.filter((d) => d.type === 'FINISH_TO_FINISH'), [items]);
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            <div className="flex items-center justify-between gap-2 shrink-0 mb-3">
                <h3 className="text-base font-semibold text-zinc-900">Related</h3>
                ...
                {customTypes.length === 0 ? (
                    <div className="rounded-lg border border-zinc-200 overflow-hidden bg-white">
                        <div className="p-4 text-center text-sm text-zinc-500">
                            No custom relationships. Add one in the &quot;More&quot; panel.
                        </div>
                    </div>
                ) : (
                    customTypes.map((ct) => (
            ...
            <TaskPickerModal open={pickerOpen} onOpenChange={setPickerOpen} taskId={taskId} workspaceId={workspaceId} dependencyType="FINISH_TO_FINISH" onSelect={handleSelect} />
        </div>
    );
}
