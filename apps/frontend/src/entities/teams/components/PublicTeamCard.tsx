      ...
      <div className="relative z-10">
        <CardHeader className="p-4 sm:p-6 pb-3 sm:pb-4">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
            ...
            {(team.industry || []).slice(0, isCompact ? 1 : 2).map((ind: string) => (
        ...
        <CardContent className="p-4 sm:p-6 pt-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-3">
              <button onClick={handleInterest} disabled={!currentUser?.id || toggleInterestMutation.isPending} className={`group/btn relative flex items-center justify-center gap-2 p-3 rounded-full font-semibold text-sm transition-all duration-300 shadow-md hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed ${isInterested ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white hover:from-cyan-600 hover:to-blue-600' : 'bg-white text-cyan-600 border-2 border-cyan-500 hover:bg-cyan-50'}`} aria-label={isInterested ? "Interested" : "Mark as interested"}>
                <Heart className={`h-4 w-4 ${isInterested ? 'fill-current' : ''}`} />
              </button>
              <button onClick={handleBookmark} className={`p-3 rounded-full transition-all duration-300 shadow-md hover:shadow-lg ${isBookmarked ? 'bg-cyan-500 text-white' : 'bg-white text-slate-600 hover:bg-slate-100 hover:text-cyan-600'}`} aria-label="Bookmark">
                <Bookmark className={`h-4 w-4 ${isBookmarked ? 'fill-current' : ''}`} />
              </button>
              <button onClick={handleShare} className="p-3 rounded-full bg-white text-slate-600 hover:bg-slate-100 hover:text-cyan-600 transition-all duration-300 shadow-md hover:shadow-lg" aria-label="Share">
                <Share2 className="h-4 w-4" />
              </button>
            </div>
            <div className="flex items-center gap-2 sm:gap-2 ml-auto">
              <button onClick={handleView} className="p-2.5 sm:p-3 rounded-full bg-white text-slate-600 hover:bg-slate-100 hover:text-cyan-600 transition-all duration-300 shadow-md hover:shadow-lg" aria-label="View details">
                <ArrowRight className="h-4 w-4" />
              </button>
              <button onClick={() => setShowRequestModal(true)} disabled={!currentUser?.id} className="group/btn flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full font-semibold text-xs sm:text-sm bg-gradient-to-r from-cyan-500 to-blue-500 text-white hover:from-cyan-600 hover:to-blue-600 transition-all duration-300 shadow-md hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Send request">
