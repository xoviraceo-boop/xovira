...
- Prefer specific, deep inquiry questions over generic "Does this look good?" when configuration is partial.
`;
  ...
  // OLD: private readonly STAGE_REQUIREMENTS... (removed from class, defined globally below)
  ...
  constructor(deps: AgentBuilderDependencies) {
    ...
    this.skillInferenceService = deps.skillInferenceService;
  }
...
Follow the flow guide principles: AI-generated messages, numbered options, dynamic follow-ups, context-aware.`,
  ...
  async processMessage(
    ...
    return await tracingService.traceOperation(
      ...
      async (span) => {
        ...
        return await this.processMessageInternal(conversationId, message, userId, startTime, span);
      }
    );
  }
  ...
  private async processMessageInternal(
    ...
    try {
      ...
      await redis.expire(rateLimitKey, AGENT_CONSTANTS.RATE_LIMIT_WINDOW);
    } catch (error) {
      if (error instanceof AgentBuilderError) {
        throw error;
      }
      // Continue if rate limit check fails? NO.
      // Hardening: Fail-Closed. If we can't check limits, we assume system instability.
      console.error('[AgentBuilder] Rate limit check failed (Redis error):', error);
      throw new AgentBuilderError(
        'SYSTEM_UNAVAILABLE',
        'Rate limit check failed unavailable',
    ...
    try {
      ...
      if (intent === AGENT_CONSTANTS.INTENT.BUILDER.BUILD_OR_MODIFY) {
        // A) Config Extraction
      ...
      // STEP 1.5: Skill Inference
      // Run skill inference in parallel or sequence? 
      // It depends on context, but let's do it here.
      let skillInference: { suggestedSkills: string[], confidence: number, reasoning: string } | null = null;
      if (intent === AGENT_CONSTANTS.INTENT.BUILDER.BUILD_OR_MODIFY) {
        skillInference = await this.skillInferenceService.inferSkills(
          sanitizedMessage,
          `Current capabilities: ${enrichedState.agentDraft.capabilities?.join(', ') || 'None'}. Description: ${enrichedState.agentDraft.description || ''}`,
          BUILT_IN_SKILLS
        );
      }
      ...
      // Apply inferred skills if confidence is high (> 0.7)
      if (skillInference && skillInference.confidence > 0.7 && skillInference.suggestedSkills.length > 0) {
        const currentSkills = new Set(updatedDraft.skills || []);
        let skillsAdded = false;

        for (const skill of skillInference.suggestedSkills) {
        ...
        if (skillsAdded) {
          ...
          // Add a note to reasoning/metadata
          updatedDraft.metadata = {
            ...updatedDraft.metadata,
            lastSkillInference: skillInference,
          };
          console.log(`[AgentBuilder] Auto-assigned skills: ${skillInference.suggestedSkills.join(', ')}`);
        }
      }
...
${enrichedPrompt}`,
      ...
      await agentMetricsService.recordBuilderInteraction(
        ...
        nextStage,
        duration,
        Object.keys(extractedConfig || {}).length
      );
    ...
    try {
      ...
      if (conversation?.aiAgent) {
        ...
        if (draft.systemPrompt) {
          const promptValidation = await this.promptSandbox.validatePrompt(draft.systemPrompt, { isSystemPrompt: true });
