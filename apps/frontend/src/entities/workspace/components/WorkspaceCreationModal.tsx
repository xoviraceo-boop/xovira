...
const STATUS_OPTIONS = [
  { label: "Active", value: "active", helper: "Visible and fully usable by members." },
  { label: "Archived", value: "archived", helper: "Hidden from lists and read-only for historical context." },
] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl gap-6">
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            <Label htmlFor="workspace-status" className="text-sm font-medium text-slate-700">
              Status
            </Label>
            <Select value={form.status} onValueChange={(value) => setForm((prev) => ({ ...prev, status: value as any }))}>
              <SelectTrigger id="workspace-status" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-left text-sm text-slate-900 shadow-xs focus:border-sky-500 focus:ring-2 focus:ring-sky-200">
                <SelectValue placeholder="Choose visibility and activity" />
              </SelectTrigger>
