...
export function useMaterialList(initialScope: MaterialScope = "owned") {
	const [page, setPage] = useState(1);
	const pageSize = 12;
	const [query, setQuery] = useState("");
	const [scope, setScope] = useState<MaterialScope>(initialScope);
	...
	useEffect(() => {
		utils.material.list.prefetch({ ...listInput, page: 1 } as any);
	...
	useEffect(() => {
		if ((queryResult.data?.items?.length || 0) === pageSize) {
			utils.material.list.prefetch({ ...listInput, page: page + 1 } as any);
		}
