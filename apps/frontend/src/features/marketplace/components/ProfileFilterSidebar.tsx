...
type Props = {
  values: {
    skills: string[];
    country?: string;
    commitment?: "PART_TIME" | "FULL_TIME" | "CONTRACT" | "FLEXIBLE";
  };
            ...
            <div className="space-y-1">
              {INDUSTRY_OPTIONS.slice(0, 10).map((opt) => (
                <label key={opt.value} className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={local.skills.includes(opt.value)} onChange={() => toggleArray("skills", opt.value)} />
                  <span>{opt.label}</span>
                </label>
              ))}
            </div>
