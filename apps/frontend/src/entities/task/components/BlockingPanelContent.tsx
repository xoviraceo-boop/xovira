...
import { MinusCircle, Search, ChevronDown, ChevronRight, Plus, ExternalLink, X, Maximize2, Flag, User } from 'lucide-react';
...
interface BlockingPanelContentProps {
    taskId: string;
    workspaceId: string;
    /** Tasks that this task blocks (blockedDependencies: dep.task is the blocked task) */
    items: Array<{
        id: string;
        taskId: string;
        dependsOnId: string;
...
export function BlockingPanelContent({ taskId, workspaceId, items }: BlockingPanelContentProps) {
    ...
    const handleSelect = (selectedTaskId: string) => {
        addDependency.mutate({
            ...
            dependsOnId: taskId,
            type: 'FINISH_TO_START' as RelationshipDependencyType,
        });
    };

    const handleRemove = (depTaskId: string) => {
        removeDependency.mutate({ taskId: depTaskId, dependsOnId: taskId });
    };
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            {/* Header: Blocking title + actions */}
            <div className="flex items-center justify-between gap-2 shrink-0 mb-3">
                <h3 className="text-base font-semibold text-zinc-900">Blocking</h3>
            ...
            {/* Collapsible Blocking N section */}
            <div className="rounded-lg border border-zinc-200 overflow-hidden bg-white flex-1 min-h-0 flex flex-col">
                ...
                >
                    ...
                    <span className="text-sm font-medium text-zinc-900">Blocking {count}</span>
                </button>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-100">
                                    {items.length === 0 ? (
                                        ...
                                        items.map((dep) => {
                                            const assignee = dep.task.assignees?.[0]?.user;
                                            return (
                                                <tr key={dep.id} className="hover:bg-zinc-50/50 group">
                                                    <td className="py-2 px-3">
                                                        ...
                                                        >
                                                            {dep.task.title ?? 'Untitled'}
                                                        </a>
                                                    </td>
                                                    <td className="py-2 px-3">
                                                        <span className={dep.task.dueDate ? 'text-red-600' : 'text-zinc-500'}>
                                                            {dep.task.dueDate
                                                                ? format(new Date(dep.task.dueDate), 'MM/dd/yy')
                                                                : '—'}
                                                        </span>
                                                    </td>
                                                    <td className="py-2 px-3">
                                                        {dep.task.priority ? (
                                                            <Flag className={cn('h-3.5 w-3.5', PRIORITY_COLORS[dep.task.priority] ?? 'text-zinc-500')} />
                                                        ) : (
