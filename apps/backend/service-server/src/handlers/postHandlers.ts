...
export function registerPostHandlers(io: any, socket: Socket) {
  socket.on('post:create', async (data: CreatePostData, ack?: (err: any, response?: any) => void) => {
    ...
    try {
      ...
      try {
        // Global feed
        io.to('feed:global').emit('post:created', { post });
        // User feed (author)
        io.to(`feed:user:${userId}`).emit('post:created', { post });
        // Project feed
        if (post.project_id) {
          io.to(`feed:project:${post.project_id}`).emit('post:created', { post });
        }
        // Team feed
        if (post.team_id) {
          io.to(`feed:team:${post.team_id}`).emit('post:created', { post });
        }
        // Proposal feed
        if (post.proposal_id) {
          io.to(`feed:proposal:${post.proposal_id}`).emit('post:created', { post });
        }
      } catch (broadcastErr) {
        console.error('Error broadcasting post:created', broadcastErr);
      }
