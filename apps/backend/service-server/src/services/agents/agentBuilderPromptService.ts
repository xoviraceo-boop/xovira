...
import { ConversationState } from './agentBuilderStateService';
...
4. Generate 2-5 dynamic follow-ups per message (not quick actions)
5. Use user's actual workspace data, never generic examples
6. Maximum 7-10 numbered options per message
7. Follow-ups should be concise (max 10 words) and actionable
8. Complete configuration with minimal conversation turns

FOLLOW-UP GENERATION:
After your response, generate 2-5 follow-up options that:
- Are concise and actionable (max 10 words)
...
6. Generate 2-5 follow-ups that are concise and actionable
7. Format response as JSON with "response" and "followups" fields
8. If JSON format fails, include follow-ups at end marked [FOLLOWUPS: ...]

Respond naturally to help the user progress through agent creation with minimal conversation turns.`;
  }
